...
try {
    ...
    if (!in_array($role, $allowed_roles)) {
        echo json_encode(['success' => false, 'message' => 'Invalid role. Only tenant and landlord roles are allowed.']);
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        $stmt->close();
        $stmt = $conn->prepare("
            SELECT user_id, username, email, full_name, phone, user_type, profile_image, created_at, last_login
        ...
        if (!empty($user['profile_image']) && file_exists(__DIR__ . '/../uploads/' . $user['profile_image'])) {
            ...
            $profile_image_url = 'https://ui-avatars.com/api/?name=' . urlencode($user['full_name']) . '&background=1abc9c&color=ffffff&size=40';
        }
