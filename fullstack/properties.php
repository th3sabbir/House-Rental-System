...
// Get category counts
$cat_counts = [];
$cat_sql = "SELECT property_type, COUNT(*) as count FROM properties WHERE status = 'available' GROUP BY property_type";
if ($cat_result = $conn->query($cat_sql)) {
    while ($cat_row = $cat_result->fetch_assoc()) {
        $cat_counts[$cat_row['property_type']] = $cat_row['count'];
    }
}
...
if (isLoggedIn()) {
    ...
    $fav_sql = "SELECT property_id FROM favorites WHERE user_id = ?";
    if ($fav_stmt = $conn->prepare($fav_sql)) {
        $fav_stmt->bind_param('i', $user_id);
        $fav_stmt->execute();
        $fav_result = $fav_stmt->get_result();
        while ($fav_row = $fav_result->fetch_assoc()) {
            $user_favorites[] = $fav_row['property_id'];
        }
        $fav_stmt->close();
    }
}
...
$sql = "SELECT property_id, title, price_per_month as price, CONCAT(COALESCE(address,''), CASE WHEN city IS NOT NULL AND city!='' THEN CONCAT(', ', UPPER(city)) ELSE '' END) AS location, bedrooms AS beds, bathrooms AS baths, area_sqft AS sqft, main_image, property_type, city FROM properties WHERE status = 'available'";
        ...
        <div class="container">
            ...
            <div class="categories-grid">
                <div class="category-card active" data-category="all">
                    ...
                    <p class="count"><?php echo $total_count; ?> listings</p>
                </div>
                ...
                <div class="category-card" data-category="apartment">
                    ...
                    <h3>Apartments</h3>
                    <p class="count"><?php echo $cat_counts['apartment'] ?? 0; ?> listings</p>
                </div>
                ...
                <div class="category-card" data-category="house">
                    ...
                    <h3>Houses</h3>
                    <p class="count"><?php echo $cat_counts['house'] ?? 0; ?> listings</p>
                </div>
                ...
                <div class="category-card" data-category="studio">
                    ...
                    <h3>Studios</h3>
                    <p class="count"><?php echo $cat_counts['studio'] ?? 0; ?> listings</p>
                </div>
                ...
                <div class="category-card" data-category="villa">
                    ...
                    <h3>Villas</h3>
                    <p class="count"><?php echo $cat_counts['villa'] ?? 0; ?> listings</p>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            <div class="results-info">
                <div class="results-count">
                    Showing <span id="resultCount"><?php echo count($rows); ?></span> properties
                </div>
                <div class="sort-by">
                    <span>Sort by:</span>
                    <div class="filter-dropdown">
            ...
            <div class="properties-grid" id="propertiesGrid">
                ...
                <?php else: foreach ($rows as $row):
                    $is_favorited = in_array($row['property_id'], $user_favorites);
                ?>
                    <div class="property-card" data-property-id="<?php echo (int)$row['property_id']; ?>" data-category="<?php echo htmlspecialchars($row['property_type']) ?>" data-price="<?php echo (int)$row['price'] ?>" data-bedrooms="<?php echo (int)$row['beds'] ?>" data-city="<?php echo htmlspecialchars(strtolower($row['city'] ?? '')) ?>">
                        <div class="property-image">
                            <?php
                                $imgSrc = '';
                                if (!empty($row['main_image'])) {
                                    $mi = trim($row['main_image']);
                                    if (preg_match('/^https?:\/\//i', $mi)) {
                                        ...
                                        if (isset($_GET['debug_images']) && $_GET['debug_images'] == '1') {
                                            $tried = implode(', ', ['uploads/properties/'.$mi, 'images/'.$mi, 'img/'.$mi]);
                                            $msg = date('Y-m-d H:i:s') . " - Missing image for property_id=" . ($row['property_id'] ?? '-') . "; tried: " . $tried . PHP_EOL;
                            ...
                            ?>
                            <img src="<?php echo htmlspecialchars($imgSrc) ?>" alt="<?php echo htmlspecialchars($row['title']) ?>" loading="lazy" onerror="this.onerror=null;this.src='https://images.pexels.com/photos/1643383/pexels-photo-1643383.jpeg?auto=compress&cs=tinysrgb&w=800';">
                            <div class="property-badge">Available</div>
                            <button class="wishlist-btn <?php echo $is_favorited ? 'active' : ''; ?>" 
                                    onclick="toggleFavorite(<?php echo (int)$row['property_id']; ?>, this)"
                        ...
                        <div class="property-info">
                            ...
                            <div class="property-footer">
                                ...
                                <a href="property-details.php?id=<?php echo (int)$row['property_id'] ?>" class="view-details-btn">View Details</a>
                            </div>
                        </div>
                    </div>
